...
class DigitalTwin:
    ...
    def _apply_condition(self, machine_id: str, c: CombinedMachineCondition) -> None:
        ...
        nominal_capacity = c.capacity_units_per_hour or (spec.default_capacity_uph if spec else m.capacity_units_per_hour)
        ...
        if m.effective_capacity_units_per_hour > 0:
            m.utilization = min(1.5, m.throughput_units_per_hour / m.effective_capacity_units_per_hour)
        else:
            m.utilization = 1.0 if m.queue_length > 0 else 0.0
